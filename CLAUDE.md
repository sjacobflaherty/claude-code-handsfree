...
CI (`.github/workflows/ci.yml`) runs `npm run build`, `npx biome check`, and `npx vitest run` on macOS with Node 20 on every pull request and every push to `main`, with a 10 minute timeout, and a newer push to the same ref cancels the older run. Changes reach `main` through a pull request, squash-merged once that check is green.
